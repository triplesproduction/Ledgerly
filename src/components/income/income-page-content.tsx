...
export default function IncomePageContent() {
    ...
    const openAddModal = () => {
        setIsEditing(false);
        setFormData({
            client: "", client_id: null, client_name: "", category: "General",
            service_id: null, service_name: "", amount: "", description: "",
            date: new Date(), status: "RECEIVED", payment_method: "bank"
        });
        setIsModalOpen(true);
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        else { setIsModalOpen(false); fetchIncome(); }
    };
    ...
    const handleSave = async () => {
        ...
        if (isNaN(amountVal) || amountVal <= 0) { alert("Please enter a valid amount"); setIsSubmitting(false); return; }
        if (!formData.client.trim()) { alert("Please enter a client name"); setIsSubmitting(false); return; }
        if (!formData.service_id) { alert("Please select a service category"); setIsSubmitting(false); return; }

        const payload = {
            date: format(formData.date, "yyyy-MM-dd"),
            description: formData.description,
            category: formData.service_name || "General",
        ...
        try {
            const { error } = isEditing && currentId
                ? await supabase.from('income').update(payload).eq('id', currentId)
        ...
        <div className="min-h-screen bg-transparent text-foreground font-sans p-6 pb-24 lg:pb-6">
                                ...
                                <Blank key={month}>
                                    ...
                                    {items.map((item: any) => (
                                        <TableRow key={item.id} className="border-white/5 hover:bg-white/5 group transition-colors">
                                            ...
                                            <TableCell><span className="font-semibold text-foreground text-sm">{item.client}</span></TableCell>
                                            <TableCell><ProjectDetailPopover description={item.description} /></TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell className="text-right font-bold text-foreground font-mono tracking-tight">₹{item.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-white" onClick={() => openEditModal(item)}><Pencil size={14} /></Button>
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </Blank>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="bg-card border-white/10 text-foreground sm:max-w-[425px] p-0 gap-0 outline-none w-[95vw] max-h-[90vh] overflow-y-auto custom-scrollbar">
                    <DialogHeader className="p-6 pb-2 space-y-1"><DialogTitle>{isEditing ? "Edit Income" : "Add Income"}</DialogTitle></DialogHeader>
                    <div className="grid gap-7 px-6 py-4">
                        <div className="grid gap-2"><Label>Client Name</Label><ClientSelector value={formData.client_id} onChange={(id, name) => setFormData({ ...formData, client_id: id, client_name: name, client: name })} /></div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="grid gap-2"><Label>Amount</Label><div className="relative"><IndianRupee size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" /><Input type="number" className="bg-white/5 border-white/10 pl-9 h-10 text-white focus-visible:ring-1 focus-visible:ring-emerald-500/20" placeholder="0.00" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} /></div></div>
                            <div className="grid gap-2"><Label>Date</Label><DatePicker date={formData.date} setDate={(d) => d && setFormData({ ...formData, date: d })} className="bg-white/5 border-white/10 h-10 text-white hover:bg-zinc-900 w-full" /></div>
                        </div>
                        <div className="grid gap-2"><Label>Project Details <span className="text-zinc-500 text-xs font-normal ml-1">(Optional)</span></Label>
                            <Input className="bg-white/5 border-white/10 text-white focus-visible:ring-1 focus-visible:ring-emerald-500/20" placeholder="E.g. Website Redesign - Milestone 1" value={formData.description} onChange={(e) => { if (e.target.value.length <= 50) setFormData({ ...formData, description: e.target.value }) }} />
                            <p className="text-[10px] text-muted-foreground text-right">{formData.description.length}/50</p>
                        </div>
                        <div className="grid gap-2"><Label>Service Category <span className="text-red-500">*</span></Label><ServiceSelector value={formData.service_id} onChange={(id, name) => setFormData({ ...formData, service_id: id, service_name: name })} /></div>
                        <div className="grid gap-2"><Label>Payment Method</Label><Select value={formData.payment_method} onValueChange={(val) => setFormData({ ...formData, payment_method: val })}><SelectTrigger className="bg-white/5 border-white/10 text-white"><SelectValue placeholder="Select Method" /></SelectTrigger><SelectContent className="bg-[#16171D] border-white/10 text-white">{paymentMethods.map((method) => (<SelectItem key={method.value} value={method.value}>{method.label}</SelectItem>))}</SelectContent></Select></div>
                    </div>
                    <div className="flex flex-col sm:flex-row justify-between items-center px-6 py-5 border-t border-white/5 mt-6 gap-4">
                        {isEditing && currentId ? (<Button variant="ghost" onClick={() => handleDelete(currentId!)} className="h-11 text-red-400 hover:text-red-300 hover:bg-red-500/10 px-4 rounded-xl flex items-center gap-2 transition-colors w-full sm:w-auto justify-center"><Trash2 size={18} /><span className="font-medium">Delete</span></Button>) : (<div className="hidden sm:block"></div>)}
                        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                            <Button variant="ghost" onClick={() => setIsModalOpen(false)} className="h-11 text-zinc-400 hover:text-white hover:bg-white/5 px-6 rounded-xl font-medium transition-colors">Cancel</Button>
                            <Button onClick={handleSave} disabled={isSubmitting} className="h-11 bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-lg shadow-orange-500/20 px-8 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none">{isSubmitting ? "Saving..." : "Save Entry"}</Button>
                        </div>
                    </div>
                </DialogContent>
            </Dialog>
            <RevenueTrendsDialog open={isTrendsOpen} onOpenChange={setIsTrendsOpen} dateRange={dateRange} />
        </div >
    );
}
