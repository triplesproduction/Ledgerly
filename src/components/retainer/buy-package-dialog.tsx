  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      for (const mDateStr of monthDates) {
          ...
          // Check if ANY instance exists within this month boundary
      ...
      // 4. Cleanup old auto-generated unpaid debt (Pending/Overdue) for these overtaken instances
      if (finalInstanceIds.length > 0) {
          // Identify the month range for more broad cleanup
          const instanceMonths = instancesToUpdate.concat(instancesToInsert as any[]).map(i => i.month_date);
          const startCleanup = instanceMonths.length > 0 ? instanceMonths.sort()[0] : null;
          const endCleanup = instanceMonths.length > 0 ? instanceMonths.sort()[instanceMonths.length - 1] : null;

          // A. Delete specifically linked milestones
          await supabase.from('income').delete().in('retainer_instance_id', finalInstanceIds).in('status', ['PENDING', 'OVERDUE']);

          // B. Delete orphans (milestones with null instance_id that match client/service/month)
          if (startCleanup && endCleanup) {
              await supabase.from('income')
                  .delete()
                  .eq('client_id', contract.client_id)
                  .eq('service_id', contract.service_id)
                  .gte('date', format(startOfMonth(new Date(startCleanup)), 'yyyy-MM-dd'))
                  .lte('date', format(endOfMonth(new Date(endCleanup)), 'yyyy-MM-dd'))
                  .is('retainer_instance_id', null)
